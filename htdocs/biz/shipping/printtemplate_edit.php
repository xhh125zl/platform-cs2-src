...
if($_POST){
	...
	$Flag=$DB->Set("shop_shipping_print_template",$Data,"where usersid='".$rsBiz["Users_ID"]."' and bizid=".$_SESSION["BIZ_ID"]." and itemid=".$itemid);
	if($Flag){
		echo '<script language="javascript">alert("修改成功");window.location.href="printtemplate.php";</script>';
	}else{
    ...
    <div id="printtemplate" class="r_con_wrap">
      ...
      <form id="printtemplate_form" class="r_con_form" method="post" action="?">
        <div class="rows">
          <label>模板名称</label>
          <span class="input">
          <input name="Title" value="<?php echo $item["title"];?>" type="text" class="form_input" size="40" maxlength="100" notnull>
        ...
        <div class="rows">
          ...
          <select name="Company" notnull>
			...
			?>
            <option value="<?php echo $value["Shipping_ID"];?>"<?php echo $item["companyid"]==$value["Shipping_ID"] ? ' selected' : '';?>><?php echo $value["Shipping_Name"];?></option>
            <?php }?>
          </select>
          <font class="fc_red">*</font></span>
          <div class="clear"></div>
        ...
        <div class="rows">
          ...
          <div class="img" id="ThumbDetail"><?php echo $item["thumb"] ? '<img src="'.$item["thumb"].'" />' : '';?></div>
          </span> </span>
          <div class="clear"></div>
        </div>
        <div class="rows">
          <label>宽度</label>
          <span class="input">
          <input name="Width" value="<?php echo $item["width"];?>" type="text" class="form_input" size="10" notnull> mm
        ...
        <div class="rows">
          <label>高度</label>
          <span class="input">
          <input name="Height" value="<?php echo $item["height"];?>" type="text" class="form_input" size="10" notnull> mm
        ...
        <div class="rows">
          <label>上偏移量</label>
          <span class="input">
          <input name="Top" value="<?php echo $item["offset_top"];?>" type="text" class="form_input" size="10" notnull> mm
        ...
        <div class="rows">
          <label>左偏移量</label>
          <span class="input">
          <input name="Left" value="<?php echo $item["offset_left"];?>" type="text" class="form_input" size="10" notnull> mm
        ...
        <div class="rows">
            <label>是否启用</label>
            <span class="input">
                <input type="radio" name="Enabled" value="0"<?php echo $item["enabled"]==0 ? ' checked' : '';?> />否&nbsp;&nbsp;
                <input type="radio" name="Enabled" value="1"<?php echo $item["enabled"]==1 ? ' checked' : '';?> />是
            </span>
            <div class="clear"></div>
        </div>
        ...
        <input type="hidden" id="Thumb" name="Thumb" value="<?php echo $item["thumb"];?>" />

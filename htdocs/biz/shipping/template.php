...
{	
	...
	if($action=="del"){
		//删除快递模板
		$Flag=$DB->Del("shop_shipping_template","Users_ID='".$Users_ID."' and Template_ID=".$_GET["Template_ID"]);
	
		if($Flag)
		{
			echo '<script language="javascript">alert("删除成功");window.location="template.php";</script>';
		}else
		{
  ...
  <div class="iframe_content">
    ...
    <div class="r_nav">
      <ul>
        ...
        <li class="cur" ><a href="template.php">快递模板</a></li>
    ...
    <div id="user" class="r_con_wrap">
      ...
      <?php if(!empty($shipping_company_list)):?>
      	<a href="/biz/shipping/template_add.php" id="create_shipping_template_btn" class="btn_green btn_w_120">添加</a> 
      <?php else: ?>
	...
	<?php foreach($template_list as $key=>$template):?>
           <tr UserID="<?php echo $template['Template_ID'] ?>">
            ...
            <a href="/biz/shipping/template_edit.php?Template_ID=<?=$template['Template_ID']?>" shipping-id="<?=$template['Template_ID']?>" class="shipping_edit_btn"><img src="/static/member/images/ico/mod.gif" alt="修改" align="absmiddle"></a>
            <a href="template.php?action=del&Template_ID=<?php echo $template['Template_ID'] ?>" onClick="if(!confirm('删除后不可恢复，继续吗？')){return false};"><img src="/static/member/images/ico/del.gif" align="absmiddle" /></a></td>
          </tr>
      <?php endforeach; ?>
        </tbody>
      </table>

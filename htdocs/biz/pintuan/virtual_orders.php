  ...
  <div class="iframe_content">
    ...
    <div id="orders" class="r_con_wrap">
      ...
      <div class="detail_card">
        ...
        <input type="hidden" name="code" value="<?php echo $rsOrder["Order_Virtual_Cards"];?>" />
        ...
        <?php }?>
          <table width="100%" border="0" cellspacing="0" cellpadding="0" class="order_info">
            ...
            <tr>
              <td nowrap>消费券码：</td>
              <td><font style="color:#088704; font-size:16px; font-weight:bold; font-family:'Times New Roman'"><?php echo $rsOrder["Order_Virtual_Cards"];?></font></td>
            </tr>
            <tr>
              <td nowrap>订单备注：</td>
            ...
            <tr>
              <td></td>
              <td><?php if($rsOrder["Order_Status"]==2){?><input type="submit" class="btn_green" name="submit_button" value="确认消费" /><?php }else{?><font sty="color:#F00">此订单为“<?php echo $_STATUS[$rsOrder["Order_Status"]]?>”状态，不能消费</font><?php }?></td>
            </tr>
          </table>
        <?php if($rsOrder["Order_Status"]==2){?>
        </form>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="order_item_list">
          ...
          <?php
		    ...
		    if($rsOrder['Order_Type'] == 'pintuan'){
		        $total+=1*$v["ProductsPriceT"];
		    }else{
		        $total+=1*$v["ProductsPriceD"];
		    }
			$qty =1;
			...
			if($rsOrder['Order_Type'] == 'pintuan'){
			    echo '</td>
				<td>￥'.$v["ProductsPriceT"].'</td>
				<td>'.$v["num"].'</td>
				<td>￥'.$v["ProductsPriceT"].'</td>
			  </tr>';
			}else{
			    echo '</td>
				<td>￥'.$v["ProductsPriceD"].'</td>
				<td>'.$v["num"].'</td>
				<td>￥'.$v["ProductsPriceD"].'</td>
			  </tr>';
			}

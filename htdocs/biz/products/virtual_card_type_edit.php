...
$rsCart = $DB->GetRs("shop_virtual_card_type","*","where User_Id='".$_SESSION["Users_ID"]."' and Type_Id=".$TypeId);
// if (empty($rsCart)) { echo '<script language="javascript">alert("不存在该卡密");window.location="virtual_card.php";</script>'; exit(); }
...
if ($_POST) {
  ...
  $Flag = $DB->set('shop_virtual_card_type', $Data, "where User_ID='".$_SESSION["Users_ID"]."' and Type_Id=".$TypeId);
  if($Flag)
  {

...
if($_POST){
	...
	$item = $DB->GetRs("shop_shipping_print_template","*","where usersid='".$rsBiz["Users_ID"]."' and bizid=".$_SESSION["BIZ_ID"]." and enabled=1 and itemid=".$templateid);
	if(!$item){
	...
	if(empty($_GET["OrderID"])){
		...
		$orders = array();
		$DB->Get("user_order","Order_ID,Address_Name,Address_Detailed,Address_Mobile,Address_Province,Address_City,Address_Area","where Users_ID='".$rsBiz["Users_ID"]."' and Biz_ID=".$_SESSION["BIZ_ID"]." and Order_Status=2 and Order_ID in(".str_replace(',,',',',implode(",",$OrderID)).") order by Order_ID desc");
		while($rsOrder = $DB->fetch_assoc()){
			...
			$rsOrder["Biz_Contact"] = $rsBiz["Biz_Contact"];
			$rsOrder["Biz_RecieveArea"] = $area_seller;
			$rsOrder["Biz_Address"] = $rsBiz["Biz_Address"];
			$rsOrder["Biz_Phone"] = $rsBiz["Biz_Phone"];
			$rsOrder["Biz_Name"] = $rsBiz["Biz_Name"];
...
<table cellpadding="0" cellspacing="0" width="500">
    ...
    <tr>
        ...
        <td><input name="Contact" class="input" value="<?php echo $rsBiz["Biz_Contact"];?>"></td>
    </tr>
    <tr>
    	<td class="left">发货人电话</td>
        <td><input name="Phone" class="input" value="<?php echo $rsBiz["Biz_Phone"];?>"></td>
    </tr>
    <tr>
    	<td class="left">发货人公司</td>
        <td><input name="Name" class="input" value="<?php echo $rsBiz["Biz_Name"];?>"></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td><input name="Address" class="input" value="<?php echo $rsBiz["Biz_Address"];?>"></td>
    </tr>
    <tr>

...
if (!function_exists('get_dsaccount_by_id')) {
	function get_dsaccount_by_id($UsersID, $User_ID) {
		if($User_ID == 0){			
			...
			$account_model = model('shop_distribute_account', 'common');
...
if (!function_exists('add_distribute_record')) {
	...
	function add_distribute_record($UsersID, $OwnerID, $Product_Price, $ProductID, $Qty, $OrderID, $Profit, $CartID) {
		...
		$Product = model('shop_products')->field('Products_ID,Products_PriceX,Products_Distributes,Biz_ID,Products_Name,commission_ratio')->where(array('Users_ID' => $UsersID, 'Products_ID' => $ProductID))->find();
		...
		//增加分销记录
		$dis_record_mode = model('shop_distribute_record');
...
if (!function_exists('change_dsaccount_record_status')) {
	...
	function change_dsaccount_record_status($OrderID, $Status) {
	    $shop_distribute_account_record = model('shop_distribute_account_record');
		$disAccountRecords = model()->query('SELECT * FROM shop_distribute_account_record as dis_account_record RIGHT JOIN shop_distribute_record as dis_record ON dis_account_record.Ds_Record_ID = dis_record.Record_ID LEFT JOIN user_order AS orders ON orders.Order_ID = dis_record.Order_ID WHERE orders.Order_ID = ' . $OrderID, 'SELECT');
		$flag = true;
		if($disAccountRecords) {
		    $record_ids = array();
			$dis_records = model('shop_distribute_record')->where(array('Order_ID'=>$OrderID))->select();
			if($dis_records){
			...
			if($record_ids){
				...
				if($account_record_count > 0){
			        $flag = model('shop_distribute_account_record')->where(array('Ds_Record_ID'=>$record_ids))->update(array('Record_Status'=>$Status));
				}
			}
		}
		return $flag;
...
if (!function_exists('create_distribute_acccount')) {
	...
	function create_distribute_acccount($rsConfig, $UserID, $Real_Name, $ownerid, $Account_Mobile, $status = 0) {
		...
		$dis_account_model = model('shop_distribute_account', 'common');
...
if(!function_exists('deal_distribute_fanben')) {//返本处理
	function deal_distribute_fanben($Fanben, $UserID){
		...
		$dis_account_model = model('shop_distribute_account');
		$user_model = model('user');
...
if (!function_exists('deal_distribute_fuxiao_record')) {
	...
	function deal_distribute_fuxiao_record($Fuxiao, $UsersID, $UserID, $OpenID) {
		$dis_account = model('shop_distribute_account', 'common')->where(array('Users_ID' => $UsersID, 'User_ID' => $UserID))->find();
		$fuxiao_model = model('shop_distribute_fuxiao');
		if(!empty($dis_account)){
	...
	function distribute_fuxiao_tixing($rsConfig){
		...
		if($now<=$notice_end && $now>=$notice_start){//提醒时间段
			...
			if(!empty($list)){
				...
				model('shop_distribute_fuxiao')->where(array('Record_ID'=>array_keys($list)))->update($sql);
				//消息发送
	...
	function distribute_dongjie_action($rsConfig) {
		...
		if($now >= $notice_start){
			...
			if(!empty($list)) {
				...
				model('shop_distribute_account')->where(array('Account_ID'=>$counts))->update('Is_Dongjie=1');
			}
		}
	}
}
	...
	function distribute_delete_action($rsConfig){
		...
		if($now >= $notice_start){
			...
			if(!empty($list)){
				...
				model('shop_distribute_account')->where(array('Account_ID'=>$counts))->update('Is_Delete=1');
...
if (!function_exists('distribute_fuxiao_return_action')) {
	...
	function distribute_fuxiao_return_action($Rules, $rsAccount, $OpenID){
		...
		if($total >= $Fuxiao[0] && $rsAccount['Is_Dongjie'] == 1){//解冻
		    model('shop_distribute_account')->where(array('Account_ID'=>$rsAccount['Account_ID']))->update(array('Is_Dongjie'=>0));				
		}
		...
		$rsRecord = model('shop_distribute_fuxiao')->field('Record_ID,Fuxiao_Count')->where($condition)->find();
		...
		if($rsRecord) {//存在记录，更改记录
			...
			model('shop_distribute_fuxiao')->where(array('Account_ID'=>$rsRecord['Record_ID']))->update($Data);
		}else {//不存在记录，则增加记录
			$Data = array(
				'Users_ID' => $UsersID,
				'User_ID' => $UserID,
			...
			model('shop_distribute_fuxiao')->insert($Data);
		}
	}
}
...
if (!function_exists('get_dis_pro_title')) {
    function get_dis_pro_title($UsersID, $type = 'front') {
		$dis_config = model('shop_distribute_config')->field('Pro_Title_Level')->where(array('Users_ID'=>$UsersID))->find();
		$pro_titles = false;
		if (!empty($dis_config)) {
...
if (!function_exists('get_my_leiji_income')) {
	...
	function get_my_leiji_income($UsersID, $UserID) {
        $record_list = model('shop_distribute_account_record')->field('Record_Money')->where(array('Users_ID' => $UsersID, 'User_ID' => $UserID,'Record_Type'=>0))->select();
...
if (!function_exists('get_my_leiji_sales')) {
	...
	function get_my_leiji_sales($UsersID, $UserID, $posterity) {
        $shop_distribute_record = model('shop_distribute_record');
		$total_sales = 0;
...
if (!function_exists('getPosterity')) {
	...
	function getPosterity($Users_ID, $User_ID, $level) {
		$shop_distribute_account = model('shop_distribute_account');
		//获取分销父路径中包含此用户ID的分销商
...
function delete_distribute_record($UsersID, $OrderID) {
	...
	if($record_list){
		...
		model('shop_distribute_account_record')->where($condition)->delete();
	}
}
 ...
 *
 * 获取数据：
 *    如果从$_POST或者$_GET中获取，使用$input->getVar($_POST['field']);，从数据库或者文件就不需要了。
 *    或者直接使用 $input->magicQuotes来消除所有的magic_quotes_gpc转义。
 *
 * 存储过程：
 *    经过$input->getVar($_POST['field'])获得的数据，就是干净的数据，可以直接保存。
...
class input {
    ...
    private $_input = array('get','post','request','env','server','cookie','session','globals','config','lang','call');
    //html标签设置
    ...
    public function __call($type, $args = array()) {
        ...
        if(in_array($type, $this->_input, true)) {
            switch($type) {
     ...
     * @return string
     +----------------------------------------------------------
     */
    public function forShow($string) {
        return $this->nl2Br( $this->hsc($string) );
    }
     ...
     * @return string
     +----------------------------------------------------------
     */
    public function forTarea($string) {
    ...
    public function makeLink($string) {
        ...
        $replacements = array(
                        "'\\1<a href=\"\\2://\\3\" title=\"\\2://\\3\" rel=\"external\">\\2://'.$this->truncate( '\\3' ).'</a>'",
                        "'\\1<a href=\"http://www.\\2.\\3\" title=\"www.\\2.\\3\" rel=\"external\">'.$this->truncate( 'www.\\2.\\3' ).'</a>'",
                        "'\\1<a href=\"ftp://ftp.\\2.\\3\" title=\"ftp.\\2.\\3\" rel=\"external\">'.$this->truncate( 'ftp.\\2.\\3' ).'</a>'",
                        "'\\1<a href=\"mailto:\\2@\\3\" title=\"\\2@\\3\">'.$this->truncate( '\\2@\\3' ).'</a>'");
    ...
    public function getVar($string) {
        return $this->stripSlashes($string);
    }
     ...
     * @return string
     +----------------------------------------------------------
     */
    public function safeHtml($text, $allowTags = null) {
        $text = trim($text);
        //完全过滤注释
        ...
        if( empty($allowTags) ) { $allowTags = $this->htmlTags['allow']; }
        //允许的HTML标签
        $text = preg_replace('/<('.$allowTags.')( [^><\[\]]*)>/i', '[\1\2]', $text);
        //过滤多余html
        if ( empty($banTag) ) { $banTag = $this->htmlTags['ban']; }
     ...
     * @return string
     +----------------------------------------------------------
     */
    public function deleteHtmlTags($string) {
        while(strstr($string, '>')) {
	...
	public function stripslashes_deep($value) {
		if ( is_array($value) ) {
			$value = array_map(function($n) {return $this->stripslashes_deep($n);}, $value);

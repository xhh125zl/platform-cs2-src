...
$item=$DB->GetRs("slide","*","where id=".$id);
if(empty($item)){
	echo '<script language="javascript">alert("此图片已经不存在！");window.location="javascript:history.back()";</script>';
	exit();
}
if($_POST){
	$Data=array(
	...
	$flag=$DB->Set("slide",$Data,"where id=".$id);
	if($flag){
  ...
  <div class="iframe_content">
    ...
    <div class="r_con_wrap">
		<form class="r_con_form" method="post" action="?ID=<?php echo $id;?>">
        	<div class="rows">
                <label>图片链接</label>
                <span class="input"><input type="text" name="linkurl" value="<?php echo $item["linkurl"];?>" size="30" class="form_input" /> 注：已“http://”开头</span>
                <div class="clear"></div>
            </div>
            ...
            <div class="rows">
              ...
              <div class="img" id="ImgDetail" style="padding-top:8px;"><?php echo $item["thumb"] ? '<img src="'.$item["thumb"].'" />' : '';?></div>
              </span></span>
              <div class="clear"></div>
            </div>
            ...
            <input type="hidden" name="Img" id="Img" value="<?php echo $item["thumb"];?>" />
        </form>
    </div>
  </div>
</div>

...
if ($wuliu) {
    ...
    if ($shipping) {
        ...
        $Order_Shipping = array(
            "Express" => $shipping["Shipping_Name"],
            "Price" => $shipp_TotalPrice ? $shipp_TotalPrice : 0
        );
...
if ($pintuan_shops["is_vgoods"] == 0) { // 实物订单
    if ($pintuan_shops['is_One'] == 1) { // 团购和单购的判断 1：团购 0 单购
        if ($pintuan_shops['is_Draw'] == 0) { // 抽奖和不抽奖的判断 0:抽奖1:不抽奖
            ...
            if ($Flag && $flags) {
                ...
                $pintuan_arr = json_encode(array(
                    'code' => '1001',
                    'url' => '/api/' . $Users_ID . '/pintuan/cart/payment/' . $order_id . '/'
                ), JSON_UNESCAPED_UNICODE);
                removeCar();
        ...
        if ($Flag && $flags) {
            $pintuan_arr = json_encode(array(
                'code' => '1001',
                'url' => '/api/' . $Users_ID . '/pintuan/cart/payment/' . $neworderid . '/'
            ), JSON_UNESCAPED_UNICODE);
            removeCar();
            Stock($ProductsID, $Users_ID);
            commit_trans();

    ...
    if (isset($_POST['goodsid']) && isset($_POST['UsersID'])) {
        ...
        if ($goodsInfo) {
            ...
            if (isset($_SESSION[$sessionid . '_cart']) && ! empty($_SESSION[$sessionid . '_cart'])) {
                ...
                $_SESSION[$sessionid . '_cart'] = serialize([
                    [
                        'id' => $id
                    ]
                ]);
...
if ($action == 'pttrue') {
    ...
    mysql_query("SET AUTOCOMMIT=0");
    $flag = $DB->set("user_order", 'Order_Status=4', 'where Users_ID="' . $User_ID . '" and Order_ID="' . $order_id . '"');
    $flags = $DB->set("pintuan_order", "order_status=4", "where order_id='" . $order_id . "'and Users_ID='" . $User_ID . "'");
    if ($flag && $flag) {
        mysql_query("COMMIT");
        $pintuan_arr = json_encode(array(
            'code' => '1001'
        ), JSON_UNESCAPED_UNICODE);
        echo $pintuan_arr;
        exit();
...
function checkout_property($request)
{
    ...
    if ($Products_ID == 0) {
        ...
        foreach ($form_Seriall as $kfs => $vfs) {
            ...
            $form_Serialls[] = array(
                $new_array["$vfs"][0] => $new_array[$vfs][1]
            );
        }

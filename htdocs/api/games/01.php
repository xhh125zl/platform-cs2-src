<div id="result">
	...
	<div class="result_f">
		<div>酷！<?php echo empty($JSON["dongzuo"]) ? "吃" : $JSON["dongzuo"];?>了<?php echo $rsResult["Score"];?><?php echo empty($JSON["unit"]) ? "个" : $JSON["unit"];?><?php echo empty($JSON["name"]) ? "苹果" : $JSON["name"];?></div>
	</div>
	<ul class="link">
...
<div id="rank">
	...
	?>
		<li<?php if($i%2==1){?> class="f"<?php }?>>
			...
			<div class="wheel"><?php echo empty($JSON["dongzuo"]) ? "吃" : $JSON["dongzuo"];?>了<?php echo $v;?><?php echo empty($JSON["unit"]) ? "个" : $JSON["unit"];?><?php echo empty($JSON["name"]) ? "苹果" : $JSON["name"];?></div>
		</li>
	<?php }?>	
	</ul>
</div>
...
$rsCategory = $DB->get("shop_category","Category_Name,Category_ID","where Users_ID='".$UsersID."' and Category_IndexShow=1 and Category_ParentID=0 order by Category_Index asc ");
...
if(!empty($_POST)){
	if(empty($_POST['cate_id'])){
		...
		$catid = $_POST['cate_id'];
		$new_products = array();
		$counts = $DB->GetRs("shop_products","count(Products_ID) as count","where Products_SoldOut=0 and Products_Category like '%,".$catid.",%' and Products_Status=1");
		$num = 4;//每页记录数
		if($counts['count'] <= $num){
			$show_load_btn = 0;
		}
...
<div id="shop_page_contents">
  ...
  <?php foreach($category_list as $k => $t){?>
  <div class="index_products">

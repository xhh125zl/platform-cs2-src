<?php
$Dwidth = array('640','320');
$DHeight = array('1010','112');
...
$(function(){
	...
	$('#web_skin_index .banner *').height($(window).height());
});
</script>
...
<div id="web_skin_menu">
    ...
    <?php
				...
				while($rsColumn=$DB->fetch_assoc()){
					...
					echo '<li class="m'.$i.'">
            <label></label>
            <div><img src="'.$rsColumn["Column_ImgPath"].'" /></div>
...
<footer id="footer">
	<ul>
     <?php
				...
				while($rsColumn=$DB->fetch_assoc()){
					$j++;
					$class= $j==1 ? ' class="first"' : '';
					echo '<li'.$class.'><a href="'.(empty($rsColumn["Column_Link"])?'/api/'.$UsersID.'/web/column/'.$rsColumn["Column_ID"].'/':$rsColumn["Column_LinkUrl"]).'"><img src="/static/api/web/skin/default/images/nav-dot.png" />'.$rsColumn["Column_Name"].'</a></li>';
			}?>
	</ul>
</footer>
<?php if($rsConfig["PagesShow"]){?>

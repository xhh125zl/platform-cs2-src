 ...
 <ul id="skin_index_menu">
  			<?php
				...
				while($rsColumn=$DB->fetch_assoc()){
					echo '<li><div class="bg"></div><a href="'.(empty($rsColumn["Column_Link"])?'/api/'.$UsersID.'/web/column/'.$rsColumn["Column_ID"].'/':$rsColumn["Column_LinkUrl"]).'"><img src="'.$rsColumn["Column_ImgPath"].'" align="absmiddle" /><br />'.$rsColumn["Column_Name"].'</a></li>';
			}?>
 </ul>
</div>

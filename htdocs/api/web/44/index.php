...
<div id="web_skin_index">
	...
	<div class="web_contents">
        ...
        <?php
	 	$i=0;
		$DB->get("web_column","*","where Users_ID='".$UsersID."' and Column_ParentID=0 order by Column_Index asc limit 0,6");
		$bg = array('style="background:#F7AB3B"','style="background:#3B87F7"','style="background:#3BD7F7"','style="background:#3BD7F7"','style="background:#3B87F7"','style="background:#F7AB3B"');
		while($rsColumn=$DB->fetch_assoc()){
			$html = $i%2==0 ? '<div class="menu_01" '.$bg[$i].'>' : '<div class="menu_01 mar_l10" '.$bg[$i].'>';
			echo $html.'<a href="'.(empty($rsColumn["Column_Link"])?'/api/'.$UsersID.'/web/column/'.$rsColumn["Column_ID"].'/':$rsColumn["Column_LinkUrl"]).'"><div class="img"><img src="'.$rsColumn["Column_ImgPath"].'" align="absmiddle" /></div><div class="text">'.$rsColumn["Column_Name"].'</div></a></div>';
			$i++;
	}?>
	   </div>
	</div>

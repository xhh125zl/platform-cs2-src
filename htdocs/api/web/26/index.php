 ...
 <div id="nav_links">
    ...
    <?php
		...
		while($rsColumn=$DB->fetch_assoc()){
			echo '<li><a href="'.(empty($rsColumn["Column_Link"])?'/api/'.$UsersID.'/web/column/'.$rsColumn["Column_ID"].'/':$rsColumn["Column_LinkUrl"]).'"><label><img src="'.$rsColumn["Column_ImgPath"].'" /></label><div>'.$rsColumn["Column_Name"].'</div></a></li>';
	}?>
    </ul>
 </div>
</div>

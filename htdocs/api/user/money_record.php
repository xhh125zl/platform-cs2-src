...
<div id="integral_record">
  <ul>
    <?php $DB->get("user_money_record","*","where Users_ID='".$UsersID."' and User_ID=".$_SESSION[$UsersID."User_ID"]." order by Item_ID desc",20);
	while($rsRecord=$DB->fetch_assoc()){
		echo '<li>【'.date("Y/m/d",$rsRecord["CreateTime"]).'】'.$rsRecord["Note"].'</li>';
	}?>
  </ul>
</div>

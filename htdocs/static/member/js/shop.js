...
var shop_obj={
	...
	home_init:function(){
		...
		$('.item .url_select .btn_select_url').click(function(){
			global_obj.create_layer('选择链接', '/member/material/sysurl.php?dialog=1&input=shop_home_url_'+$(this).attr("ret"),1000,500);
		});
	...
	products_list_init:function(){
		...
		$(".mousehand").each(function(){
                $(this).click(function(){
                 global_obj.create_layer('商品佣金详情', '/member/distribute/levelview.php?ProductsID='+$(this).attr('id'),971,500,0,0);
                })
        })
	},
	...
	dis_config_init:function(){
		//分销模式选择
		$("input[name=Distribute_Method]").click(function(){
			var method_val = $(this).attr("value");
			for(var i=0;i<2;i++){
				if(i==method_val){
					$('#dis_method_'+method_val).show();
				}else{
					$('#dis_method_'+i).hide();
				}
			}
		...
		$('#menubox').on('click','.btn_select_url',function(){
			...
			global_obj.create_layer('选择链接', '/member/material/sysurl.php?dialog=1&input=distribute_url_'+$(this).attr("ret"),1000,500);
		});
		
		$("input[name=Dis_Agent_Type]").click(function(){
			var region_aget = "省%<input name=\"Agent_Rate[Province]\" value=\"0\" class=\"form_input\" size=\"3\" maxlength=\"10\" type=\"text\"> 市%<input name=\"Agent_Rate[City]\" value=\"0\" class=\"form_input\" size=\"3\" maxlength=\"10\" type=\"text\">";
	...
	//---------- diy页面 ----------//
	diy_init:function(){
		shop_obj.firstrproperty = $(".ps2").html(); //初始化信息
		...
		//------ 组件：搜索框p12上传 -------//
		global_obj.file_upload($("#upfile_search"),'','',1,true,1,function(filename,filepath){
			if($(".selectObj").hasClass(pName)){
				$(".selectObj .dragPart .submit").html('<img src="'+filepath+'" width="40" />');
			}
		});
	},
	//组件元素操作
	...
	"p6"	: { //一行四列
		"packageElement"	: function(packageName){
			...
			$str +=			"</div>";
			$str +=  		"<div class='p6ImgFrame'>";
			$str +=				"<div class='imgObj'>图片区域4</div>";
			$str +=				"<div class='wordObj'>文字区域4</div>";
			$str +=				"<div class='clean'></div>";
			$str +=			"</div>";
			$str +=		"</div>";
	...
	"p7"	: { //一行五列
		"packageElement"	: function(packageName){
			...
			$str +=			"</div>";
			$str +=  		"<div class='p7ImgFrame'>";
			$str +=				"<div class='imgObj'>图片区域5</div>";
			$str +=				"<div class='wordObj'>文字区域5</div>";
			$str +=				"<div class='clean'></div>";
			$str +=			"</div>";
			$str +=		"</div>";
	...
	"p12"	: { //搜索框组件
		"packageElement"	: function(packageName){
			$str  = "";
			$str += "<div class='p12 sprite1' packageName='"+packageName+"' background0='' color0=''>";
			$str += 	"<div class='dragPart'>";
			$str +=  		"<div class='p12ImgFrame'>";
			$str +=				"<div class='submit'><img src='/static/api/shop/skin/1/search.jpg' /></div>";
			$str +=				"<input type='text' class='input' />";
			$str +=				"<div class='clean'></div>";
			$str +=			"</div>";
			$str +=		"</div>";
			$str +=		"<div class='delObj hand' onclick='shop_obj.delObjEvt(this,\"p12\");'><img "+shop_obj.Evt1+" src='"+shop_obj.cloIco+"' /></div>";
			$str +=		"<div class='clean'></div>";
			$str +=	"</div>";
			return $str;
		},
		"insertWord"	: function(packageName){ //文字编辑
			var $value = $(".ps2_frmae_p12 .img_name input").val();
			if($(".selectObj").hasClass(packageName)){
				if($value){
					$(".selectObj .wordObj").show().text($value);
				} else {
	...
	"showproElement": function(packageName){ //显示属性编辑面板
		switch(packageName)
		{
			...
			case "p12"://搜索框
				...
				$color0 = $color0?$color0:"#ffffff";
				$bg0    = $bg0?$bg0:"#ED145B";				
				$("#colorSelectorWordp12_0").val($color0).css("background",$color0);
				$("#colorSelectorBgp12_0").val($bg0).css("background",$bg0);
			break;
	...
	"colorPicker": function(){
		...
		//p12 搜索框
		$('#colorSelectorWordp12_0').ColorPicker({
			onSubmit: function(hsb, hex, rgb, el) {
				var $id   = $(el).attr("id");
				$id = $id.toString();
				var $num  = $id.replace("colorSelectorWordp12_","");
				$(el).val("#"+hex).css("background","#"+hex);
				$(el).ColorPickerHide();
				$(".selectObj .input").eq($num).css("border","1px solid #"+hex);
				$(".selectObj").attr("color"+$num,"#"+hex);
			}
		});
		$('#colorSelectorBgp12_0').ColorPicker({
			onSubmit: function(hsb, hex, rgb, el) {
				var $id   = $(el).attr("id");
				$id = $id.toString();
				var $num  = $id.replace("colorSelectorBgp12_","");
				$(el).val("#"+hex).css("background","#"+hex);
				$(el).ColorPickerHide();
				$(".selectObj .p12ImgFrame").css("background","#"+hex);
				$(".selectObj").attr("background"+$num,"#"+hex);
			}
		});
	},
	"btnCtrl"	: function(){
		...
		$(".ipad .sprite1").each(function(index, element) {
			...
			switch($packageName){
				...
				case "p12": //搜索框					
					var $img0		 = $(this).find(".p12ImgFrame").find(".submit img").attr("src");
					var $color0		 = $(this).attr("color0");
					var $bg0		 = $(this).attr("background0");
						str += "{";
						str += 		'"type" : "p12",';

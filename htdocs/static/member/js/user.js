		...
		$('.upd_money').dblclick(function(){
			...
			o.children('input, select').keyup(function(event){
				if(event.which==13){
					...
					$.post('/member/user/user_list.php?', "action=money_mod&UserID="+o.parent().parent().attr('UserID')+'&field='+o.parent().attr('field')+'&Value='+value, function(data){
						if(data.status==1){
							var msg='修改成功！';
							o.html(parseFloat(o.data('text'))+value);							
						}else if(data.status==2){

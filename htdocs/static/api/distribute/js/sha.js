var sha_proxy_obj = {	
	sha_proxy_init:function(){
		...
		$('.pay_select_list a#zfb,.pay_select_list a#wzf').click(function(){
			...
			$.post(ajax_url,'action=get_sha_pay_method&OrderID='+orderid+'&method='+$(this).attr('data-value'),function(data){
				if(data.status==1){
		...
		$('.pay_select_list button').click(function(){
			...
			//$(this).attr('disabled', true);
			$.post(ajax_url,'action=deal_sha_order_pay&OrderID='+orderid+'&password='+password,function(data){
				if(data.status==1){
					global_obj.win_alert(data.msg, function() {
                        window.location.href=data.url;
                    });

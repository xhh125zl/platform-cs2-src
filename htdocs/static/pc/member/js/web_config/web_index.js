...
function update_data(id) {//更新
	var get_text = $.ajax({
		type: "POST",
		url: SITE_URL+'/pc.php/member/web_api/code_update',
...
function get_goods_class() {//查询子分类
	...
	if (gc_id > 0) {
		$.get(SITE_URL+'/pc.php/member/web_api/category_list/id/' + gc_id, function(data) {
			$(data).each(function(){
				var obj = $(this);
...
function get_recommend_goods() {//查询商品
	...
	if (gc_id > 0 && goods_name != '') {
		$("#show_recommend_goods_list").load(SITE_URL+'/pc.php/member/web_api/recommend_list/id/' + gc_id + '/goods_name/' + goods_name);
	}
}
	...
	for (var i = 1; i <= recommend_max; i++) {//防止数组下标重复
		if ($(".right dl[recommend_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			add_html = '<dl recommend_id="'+i+'"><dt><h4>商品推荐</h4>'+del_append+
    			'<a href="JavaScript:show_recommend_dialog('+i+');"><i class="icon-shopping-cart"></i>商品块</a></dt>'+
...
function update_pic(id, pic) {//更新图片
	if (id=='tit') {
	    ...
	    if (get_type=='txt') {
			$('#picture_floor').html('<h2>'+tit_title+'</h2><span>'+tit_floor+'</span>')
		}
	}
	var obj = $("#picture_"+id);
...
function add_slide_adv() {//增加图片
	for (var i = 1; i <= slide_pic_max; i++) {//防止数组下标重复
		if ($("#upload_adv_form ul li[slide_adv_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			');"></span><span class="thumb size-106x106" onclick="select_slide_adv('+i+');"><img src="'+SITE_URL+
			'/static/pc/member/images/picture.gif" /></span></div>'+text_input+'</li>';
...
function select_recommend_goods(goods_id) {//商品选择
	...
	text_append += '<a href="'+SITE_URL+'/pc.php/shop/goods/index/id/'+goods_id+'" target="_blank">';

...
$rsOrder = $DB->GetRs('sha_order','*','where Order_ID='.$OrderID);
...
$jsApi = new JsApi_pub();
if (!isset($_GET['code'])){
	$url = $jsApi->createOauthUrlForCode("http://".$_SERVER['HTTP_HOST']."/pay/wxpay2/sendto_sha.php?UsersID=".$UsersID."_".$OrderID);
	Header("Location: $url");
}else{
	$code = $_GET['code'];
	$jsApi->setCode($code);

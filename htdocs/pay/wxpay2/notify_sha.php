...
if(isset($GLOBALS['HTTP_RAW_POST_DATA'])){
	...
	$rsOrder=$DB->GetRs("sha_order","Users_ID,User_ID,Order_Status","where Order_ID='".$OrderID."'");
	if(!$rsOrder){
		echo "订单不存在";
		exit;
	}
	...
	$rsOrder=$DB->GetRs("sha_order","Users_ID,Order_Status","where Order_ID='".$OrderID."'");
	if(!$rsOrder){
		echo "订单不存在";
		exit;
	}

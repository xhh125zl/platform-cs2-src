...
$rsOrder = $DB->GetRs('distribute_order','*','where Order_ID='.$OrderID);
$pay_fee = $rsOrder["Order_TotalPrice"];
$pay_orderno = strval($rsOrder["Order_CreateTime"].$OrderID);
$pay_subject = '购买'.$rsOrder['Level_Name'];
...
//服务器异步通知页面路径
$notify_url = "http://".$_SERVER['HTTP_HOST']."/pay/alipay/notify_distribute.php";
//需http://格式的完整路径，不允许加?id=123这类自定义参数

//页面跳转同步通知页面路径
$call_back_url = "http://".$_SERVER['HTTP_HOST']."/pay/alipay/call_back_distribute.php";
//需http://格式的完整路径，不允许加?id=123这类自定义参数
...
//付款金额
$total_fee = strval(floatval($pay_fee));
//必填

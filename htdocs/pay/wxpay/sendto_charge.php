...
$rsCharge=$DB->GetRs("user_charge","*","where Users_ID='".$UsersID."' and User_ID='".$_SESSION["User_ID"]."' and Item_ID='".$OrderID."'");
$pay_fee = strval(floatval($rsCharge["Amount"])*100);
$pay_orderno = time().strval($OrderID);
$pay_subject = "(会员:".$_SESSION["User_ID"].")在线充值，充值编号:".$OrderID;
...
$wxPayHelper->setParameter("notify_url", "http://".$_SERVER['HTTP_HOST']."/pay/wxpay/notify_charge.php");
...
function callpay()
{
	...
	if(res.err_msg=='get_brand_wcpay_request:ok'){
		...
		setTimeout("window.location.href = 'http://<?php echo $_SERVER['HTTP_HOST'];?>/pay/wxpay/notify_charge.php?OrderID=<?php echo $OrderID;?>'",2000);
	}else{

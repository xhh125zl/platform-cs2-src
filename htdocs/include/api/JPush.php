<?php
class Jpush_send{
    private $app_key = 'd50e6263a8dca9006527a9b3';        //待发送的应用程序(appKey)，只能填一个。
        private $master_secret = 'd54cdd87991cc4870d5a9424';    //主密码
         ...
         **/
        //调用推送方法
        public function send_pub($receive,$content,$m_type = '',$m_txt = '',$m_time = 86400){
            $m_type = 'http';//推送附加字段的类型
            $m_txt = '【好分销】';//推送附加字段的类型对应的内容(可不填) 可能是url,可能是一段文字。
            $m_time = '86400';//离线保留时间
                    ...
                    $error_code=$res_arr['error']['code'];             //错误码
                        switch ($error_code) {
                            ...
                            case 1030:
                                ...
                                break;
                            default:
                                $message= '失败(返回其他状态，目前不清楚，请联系开发人员！)';
                                break;
                        }
                }else{
                    $message="发送成功！";
            ...
            $msg_data = [
                'content' => $content,
                'create_time' => time(),
                'send_author' => $_SESSION['Users_Account']
            ];
            $flag = $DB->Add('msg_push', $msg_data);
            echo  "推送信息:" . $message;
        }

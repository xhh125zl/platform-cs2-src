...
class OrderObserver {
	...
	private function handle_dis_sha_info()
	{
		global $DB1;
		$order = $this->order;
	    $Sha_Rate_Obj = $DB1->Get('distribute_account', 'Account_ID,Real_Name,Account_Mobile', ' where `Users_ID`="' .$order->Users_ID. '" AND `Enable_Agent`=1');
		$Sha_Rate_Count = array();
	    $SHA_Account_Id_List = ',';
	    while ($row = $DB1->fetch_assoc()) 
	    {
	    	$Sha_Rate_Count[] = $row;
	    	$SHA_Account_Id_List .= $row['Account_ID'].',';
	    }
	    ...
	    if (!empty($cartProduct)) 
	    {
	    	foreach ($cartProduct as $k => $v) 
		    {
		    	foreach ($v as $p => $productInfo) 
		    	{
		    		...
		    		{
		    			//当前商品的股东分红佣金
	    				$sha_record_monery = ($productInfo['ProductsProfit']*($productInfo['platForm_Income_Reward']/100)*($productInfo['sha_Reward']/100));
						if (!empty($Sha_Rate_Count)) 
	    				{
	    					foreach ($Sha_Rate_Count as $key => $value) 
	    					{
	    						$flag_a = $this->do_agent_award($value['Account_ID'], $sha_record_monery/count($Sha_Rate_Count));
	    					}
	    				}
	    				
	    				$flag_b = $this->do_sha_award_record($accountinfo, $sha_record_monery, $productInfo, count($Sha_Rate_Count), $SHA_Account_Id_List);
		    			$flag = $flag_a && $flag_b;
		    		}

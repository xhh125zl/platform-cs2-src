...
function build_attr_html($type_id, $product_id = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
		...
		$html .= ($val['Attr_Type'] == 1 || $val['Attr_Type'] == 2) ?
             (($FinanceType == 0) ? '&nbsp;&nbsp;属性价格&nbsp;&nbsp;<input type="text" notnull name="attr_price_list[]" value="' . (empty($val['Attr_Price']) ? "0" : $val['Attr_Price']) . '" size="5" maxlength="10" />' : '&nbsp;&nbsp;属性价格&nbsp;&nbsp;<input type="text" notnull name="attr_price_list[]" value="' . (empty($val['Attr_Price']) ? "0" : $val['Attr_Price']) . '" size="5" maxlength="10" />&nbsp;&nbsp;供货价&nbsp;&nbsp;<input type="text" notnull name="attr_price_list2[]" value="' . (empty($val['Supply_Price']) ? "0" : $val['Supply_Price']) . '" size="5" maxlength="10" />') :
            ' <input type="hidden" name="attr_price_list[]" value="0" /><input type="hidden" name="attr_price_list2[]" value="0" />';
        $html .= '</td></tr>';
    }
...
function get_attr_list_by_typeid($type_id, $products_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.Attr_ID, a.Attr_Name, a.Attr_Input_Type, a.Attr_Type, a.Attr_Values, v.Attr_Value, v.Attr_Price, v.Supply_Price ".
            "FROM " .$table_attribute. " AS a ".
	...
	if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])))
    {
        ...
        if(isset($_POST['attr_id_list']))
        {
            foreach ($_POST['attr_id_list'] AS $key => $attr_id)
            {
				...
				if(isset($_POST['attr_price_list2'])){
					 
					//$attr_price2 = empty($_POST['attr_price_list2'][$key]) ? 0 : empty($_POST['attr_price_list2'][$key]); 
					$attr_price2 = empty($_POST['attr_price_list2'][$key]) ? 0 : $_POST['attr_price_list2'][$key]; 				
				}else{
					$attr_price2 = 0;
				}
			    ...
			    if (!empty($attr_value))
                {
                    if (isset($products_attr_list[$attr_id][$attr_value]))
                    {
						...
						$products_attr_list[$attr_id][$attr_value]['attr_price2'] = $attr_price2;
                    }
                    else
                    {
                        // 如果原来没有，标记为新增
        ...
        foreach ($products_attr_list as $attr_id => $attr_value_list)
        {
            foreach ($attr_value_list as $attr_value => $info)
            {
                if ($info['sign'] == 'insert')
                {
                    $sql = "INSERT INTO shop_products_attr (Attr_ID, Products_ID, Attr_Value, Attr_Price, Supply_Price)".
                            "VALUES ('$attr_id', '$product_id', '$attr_value', '$info[attr_price]', '$info[attr_price2]')";
                }
                elseif ($info['sign'] == 'update')
                {
                    $sql = "UPDATE shop_products_attr SET Attr_Price = '$info[attr_price]',Supply_Price = '$info[attr_price2]' WHERE Product_Attr_ID = '$info[product_attr_id]' LIMIT 1";
                }
                else
                {

...
function get_front_shiping_company_dropdown($UsersID,$BizConfig){
	...
	if(empty($BizConfig['Shipping'])){
		...
		global $DB1;
		$condition = "where Users_ID='".$UsersID."' and Biz_ID=".$BizConfig['Biz_ID']." and Shipping_ID in (".$ids_string.")";
		
		$rsShippings = $DB1->get('shop_shipping_company','Shipping_ID,Shipping_Name',$condition);
		$shipping_list = $DB1->toArray($rsShippings);

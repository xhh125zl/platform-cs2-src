...
class OrderObserver {
	...
	public function confirmed($order){
		...
		$disAccountRecord = model()->query('SELECT COUNT(*) as count FROM shop_distribute_account_record AS a RIGHT JOIN shop_distribute_record AS r ON a.Ds_Record_ID=r.Record_ID WHERE r.Order_ID=' . $this->order['order_id'], 'find');
	...
	private function handle_dis_record_info() {
		...
		// 将分销账号记录置为完成
		$flag_b = model('shop_distribute_account_record')->where(array('Ds_Record_ID'=>$Record_IDS))->update(['Record_Status' => 2]);
	...
	private function handle_dis_account_info() {
		...
		$distribute_record_model = model('shop_distribute_record');
		$distribute_account_model = model('shop_distribute_account');
		$distribute_records = $distribute_record_model->field('Record_ID')->where(array('Order_ID'=>$this->order['order_id']))->select();
		$Record_IDS = array();
		foreach($distribute_records as $k => $v) {
			$Record_IDS[] = $v['Record_ID'];

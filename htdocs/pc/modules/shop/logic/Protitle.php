...
class ProTitle {
	...
	*/
	function get_son_level($Pro_Title_Level = ''){
		...
		$Pro_Title_Level = empty($Pro_Title_Level) ? array() : array_reverse($Pro_Title_Level, true);
		//直销人数
		$num = model('shop_distribute_account')->where(array('invite_id'=>$this->User_ID))->total();
		$son_level = 0;
		foreach($Pro_Title_Level as $k => $v){
	...
	*/
	...
	function get_nobility_level(){
		$md = model('shop_distribute_account')->where(array('User_ID'=>$this->User_ID))->find();
		$nobility = 0;
	...
	*/
	function up_nobility_level($level = '', $Pro_Title_Level = array()) {
		...
		if($my_now_level > $level) {
			$Flag = model('shop_distribute_account')->where(array('Users_ID'=>$this->Users_ID,'User_ID'=>$this->User_ID))->update(array('Professional_Title'=>$my_now_level));
			if($Flag) {
				$data['status'] = 1;
				return $data;//级别晋升成功
			}else {

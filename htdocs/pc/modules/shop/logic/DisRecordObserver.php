...
class DisRecordObserver{
	...
	public function created($Record_ID) {
		$this->DisRecord = model('shop_distribute_record')->field('*')->where(array('Record_ID'=>$Record_ID))->find();
	...
	private function generateDistributeAccountRecord(){
		...
		foreach ($ancestors_rsort as $key => $value) {
			if ($Buyer_ID == $value) {//自销
				...
				$Record_Money = !empty($Distribute_List[$level]) ? $Distribute_List[$level] * $Qty : 0;
				$Record_Price = !empty($Distribute_List[$level]) ? $Distribute_List[$level] : 0;
			} else {
				if($Owner_ID == $value) {
				...
				$Record_Money = !empty($Distribute_List[$key]) ? $Distribute_List[$key] * $Qty : 0;
				$Record_Price = !empty($Distribute_List[$key]) ? $Distribute_List[$key] : 0;
			}

...
class backup{
	...
	private function update_distribute_money($orderid,$productid,$cartid,$qty,$type=0){
		...
		$bonus_list = model()->query("select d.* from shop_distribute_account_record as d LEFT JOIN shop_distribute_record as r ON d.Ds_Record_ID=r.Record_ID where r.Order_ID=".$orderid." and r.Product_ID=".$productid." and d.CartID=".$cartid." group by d.level", 'select');
		if(!empty($bonus_list)){
			foreach($bonus_list as $item){
				...
				$data["Owner_ID"] = 0;
				model('shop_distribute_account_record')->insert($data);
			}
		}
	}

	...
	private function pay_orders($orderid) {
		...
		if($Flag_b && $Flag_c) {
			...
			if($rsUser['is_distribute'] == 0) {//不是分销商
				...
				$tixian = 0;
				$rsAccount = model('shop_distribute_account')->field('User_ID,Enable_Tixian,Account_ID,Is_Dongjie,Is_Delete,Users_ID')->where(array('Users_ID'=>$rsOrder['users_id'],'User_ID'=>$rsOrder['user_id']))->find();
				if($rsAccount) {
					if($rsAccount['Enable_Tixian'] == 0) {
						...
						if($tixian == 1) {
							model('shop_distribute_account')->where(array('Users_ID'=>$rsOrder['users_id'],'Account_ID'=>$rsAccount['Account_ID']))->update(array('Enable_Tixian'=>1));
						}
					}

...
class distributeController extends controllController {
	...
	public function distribute_recordOp() {
		...
		if(IS_AJAX) {
			if($filter != 'all') {
				if($filter == 'self') {
				    $sql1 = 'SELECT COUNT(*) as count FROM shop_distribute_account_record as a LEFT JOIN shop_distribute_record as r ON a.Ds_Record_ID=r.Record_ID LEFT JOIN shop_products as p ON p.Products_ID=r.Product_ID WHERE a.Users_ID="' . $this->UsersID . '" AND a.User_ID=' . $_SESSION[$this->UsersID . 'User_ID'] . ' AND r.Owner_ID=' . $_SESSION[$this->UsersID . 'User_ID'] . ' AND a.Record_Type=0';
				}elseif($filter == 'down') {
				    $sql1 = 'SELECT COUNT(*) as count FROM shop_distribute_account_record as a LEFT JOIN shop_distribute_record as r ON a.Ds_Record_ID=r.Record_ID LEFT JOIN shop_products as p ON p.Products_ID=r.Product_ID WHERE a.Users_ID="' . $this->UsersID . '" AND a.User_ID=' . $_SESSION[$this->UsersID . 'User_ID'] . ' AND r.Owner_ID!=' . $_SESSION[$this->UsersID . 'User_ID'] . ' AND a.Record_Type=0';
				}
			}else {
			    $sql1 = 'SELECT COUNT(*) as count FROM shop_distribute_account_record as a LEFT JOIN shop_distribute_record as r ON a.Ds_Record_ID=r.Record_ID LEFT JOIN shop_products as p ON p.Products_ID=r.Product_ID WHERE a.Users_ID="' . $this->UsersID . '" AND a.User_ID=' . $_SESSION[$this->UsersID . 'User_ID'] . ' AND a.Record_Type=0';
			}
			...
			//计算全部个数
			$all_distribute_count = model()->query('SELECT COUNT(*) as count FROM shop_distribute_account_record WHERE Users_ID="' . $this->UsersID . '" AND User_ID=' . $_SESSION[$this->UsersID . 'User_ID'] . ' AND Record_Type=0','find');
			$this->assign('all_distribute_count', $all_distribute_count['count']);
			//自销次数
			$self_distribute_count = model()->query('SELECT COUNT(*) as count FROM shop_distribute_account_record as a LEFT JOIN shop_distribute_record as r ON a.Ds_Record_ID=r.Record_ID WHERE a.Users_ID="' . $this->UsersID . '" AND a.User_ID=' . $_SESSION[$this->UsersID . 'User_ID'] . ' AND r.Owner_ID=' . $_SESSION[$this->UsersID . 'User_ID'] . ' AND a.Record_Type=0','find');
			$this->assign('self_distribute_count', $self_distribute_count['count']);
			//下级销售次数
			$posterity_distribute_count = model()->query('SELECT COUNT(*) as count FROM shop_distribute_account_record as a LEFT JOIN shop_distribute_record as r ON a.Ds_Record_ID=r.Record_ID WHERE a.Users_ID="' . $this->UsersID . '" AND a.User_ID=' . $_SESSION[$this->UsersID . 'User_ID'] . ' AND r.Owner_ID!=' . $_SESSION[$this->UsersID . 'User_ID'] . ' AND a.Record_Type=0','find');
		...
		if(!empty($_SESSION[$this->UsersID . 'User_ID'])){
			if($rsUser){
				...
				if($rsUser['Is_Distribute']){
					$a = model('shop_distribute_account')->where(array('Users_ID'=>$this->UsersID,'User_ID'=>$_SESSION[$this->UsersID . 'User_ID']))->find();
					if($a){
	...
	public function distribute_withdrawOp() {
		...
		$dis_account_record_model = model('shop_distribute_account_record');
		if(IS_AJAX) {
            $condition = array(
			...
			if($this->rsDisAccount['Enable_Tixian'] == 0){
				if($this->shopConfig['withdraw_type'] == 0){//无限制
					...
					model('shop_distribute_account')->where(array('User_ID'=>$_SESSION[$this->UsersID . 'User_ID']))->update($data);
	...
	public function distribute_qrcodehbOp() {
		...
		if($rsUser['Is_Distribute'] == 1){
			...
			$rsAccount = model('shop_distribute_account')->field('*')->where(array('Users_ID'=>$this->UsersID,'User_ID'=>$_SESSION[$this->UsersID . 'User_ID']))->find();
	...
	public function withdraw_applyOp() {
		...
		if($this->rsDisAccount['Enable_Tixian'] == 1){
			...
			if($this->shopConfig['withdraw_type'] == 0){
				model('shop_distribute_account')->where(array('Users_ID'=>$this->UsersID,'Account_ID'=>$this->rsDisAccount['Account_ID']))->update(array('Enable_Tixian'=>1));
				header('location:' . url('distribute/distribute_withdraw'));exit;
	...
	public function pro_titleOp() {
		...
		//直销人数
		$user_count = model('shop_distribute_account')->where(array('invite_id'=>$_SESSION[$this->UsersID . 'User_ID']))->total();
		...
		$dis_account_record_list = model('shop_distribute_account_record')->field('Nobi_Money,Record_Status,Nobi_Status')->where(array('User_ID'=>$_SESSION[$this->UsersID . 'User_ID'],'Nobi_Money >'=>0))->select();

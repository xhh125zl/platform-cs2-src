...
class distribute_ajaxController extends controllController {
	...
	private function _withdraw_appy() {
		...
		$dsAccount = model('shop_distribute_account')->field('Account_ID,balance')->where($condition)->find();
		...
		if($_POST['money'] > $dsAccount['balance']) {
			...
			if($UserMethod['Method_Type'] == 'wx_hongbao' && ($money < 1 || $money > 200)){
				...
				model('shop_distribute_account')->where($condition)->update('balance=balance-' . $money);
				$Flag = model('shop_distribute_account_record')->insert($data);
				if($Flag) {
					$Data = array('status'=>1);
				}else {

...
if($_POST)
{
	...
	$Flag=$DB->Add("ad_list",$Data);
	if($Flag)
	{
		echo '<script language="javascript">alert("添加成功");window.location="ad_list.php";</script>';
	}else
	{
...
<div id="iframe_page">
  <div class="iframe_content">
    ...
    <div id="products" class="r_con_wrap">
      ...
      <form id="products_form" class="r_con_form" method="post" action="ad_add.php">
       <div class="rows">
          <label>隶属分类</label>
          <span class="input">
          <select name='IDS'>
...
foreach($ParentCategory as $key=>$value){
		echo '<option value="'.$value["AD_IDS"].'">'.$value["AD_Name"].'</option>';
}
?>
          </select>
          </span>
        ...
        <div class="rows">
          <label>广告链接</label>
          <span class="input">
          <input type="text" name="Link" value="" class="form_input" notnull style="width:600px;" />
        ...
        <div class="rows">
          <label>起止时间</label>
          <span class="input">
          <input name="Time" type="text" value="" class="form_input" size="42" readonly notnull />
          <font class="fc_red">*</font> </span>
          <div class="clear"></div>
        </div>
        <div class="rows">

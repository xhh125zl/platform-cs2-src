...
if(isset($_POST['action'])){
	if($_POST['action']=='used'){
		...
		$Flag=$DB->Set("turntable_sn",$Data,"where Users_ID='".$_SESSION['Users_ID']."' and Turntable_ID=".$_POST["TurntableID"]." and SN_ID=".$_POST['SNID']);
		if($Flag){
			$Data=array(
				"status"=>1
			);
...
$condition = "where Users_ID='".$_SESSION["Users_ID"]."' and Turntable_ID=".$_GET['TurntableID']." and SN_Code<>0 and SN_Status>0";
...
if(!empty($_GET["PrizeClass"])){
	$condition .= " and Turntable_PrizeID=".$_GET["PrizeClass"];
}
?>
<!DOCTYPE HTML>
<html>
...
<div id="iframe_page">
  <div class="iframe_content">
    ...
    <script language="javascript">$(document).ready(turntable_obj.sn_init);</script>
    <div id="sncode" class="r_con_wrap">
      ...
      <form class="search" id="search_form" method="get" action="">
        ...
        <input type="hidden" name="TurntableID" value="<?php echo $_GET['TurntableID'];?>" />
      </form>
      <table border="0" cellpadding="5" cellspacing="0" class="r_con_table">
        ...
        <tbody>
          <?php $DB->getPage("turntable_sn","*",$condition." order by SN_CreateTime desc",$pageSize=10);
		$i=1;
		...
		while($rsSN=$DB->fetch_assoc()){?>
          <tr>
            ...
            <td nowrap="nowrap"><?php echo $level[$rsSN["Turntable_PrizeID"]] ?></td>
            ...
            <td nowrap="nowrap" class="last" Data='{"SNID":"<?php echo $rsSN["SN_ID"] ?>","TurntableID":"<?php echo $rsSN["Turntable_ID"] ?>"}'><?php echo $rsSN["SN_Status"]==1?'[<a href="#used">使用</a>]':'' ?></td>
          </tr>
          <?php $i++;
		  }?>
        </tbody>

...
if($_POST){
	...
	$Flag=$DB->Set("shipping_orders",$Data,"where Users_ID='".$_SESSION["Users_ID"]."' and Orders_ID=".$OrderID);
	if($Flag){
		$url='http://'.$_SERVER["HTTP_HOST"]."/api/".$_SESSION['Users_ID']."/cloud/member/products/";
		require_once($_SERVER["DOCUMENT_ROOT"].'/include/library/weixin_message.class.php');
		$weixin_message = new weixin_message($DB,$_SESSION["Users_ID"],$shipping_orders["User_ID"]);
     ...
     <form id="orders_mod_form" method="post" action="?OrderId=<?php echo $OrderID;?>" class="r_con_form">
        ...
        <div class="rows">
          ...
          <span class="input">
                ...
                <input type="text" name="Detailed" value="<?php echo $shipping_orders["Address_Detailed"] ?>" size="35" style="height:28px; border:1px #dfdfdf solid; padding:0px 5px" />

...
if($_POST){
	...
	$Data=array(
		...
		"Products_Qrcode"=>generate_qrcode(base_url().'api/'.$_SESSION['Users_ID'].'/cloud/products/'.$rsProducts['Products_ID'].'/'),
		"Products_xiangoutimes"=>empty($_POST['xiangoutimes'])?"0":$_POST['xiangoutimes'],
		"zongrenci"=>$zongrenci,
		"Products_Status" => $_POST["Products_Status"]
	);
	
	$Flag = $DB->Set("cloud_Products",$Data,"where Users_ID='".$_SESSION["Users_ID"]."' and Products_ID=".$ProductsID);
	//跟新云购码
...
<div id="iframe_page">
	<div class="iframe_content">
		...
		<div id="products" class="r_con_wrap">
            ...
            总人次 <font style="color:#F00; font-size:12px;"><?php echo $rsProducts["zongrenci"];?></font>&nbsp;&nbsp;&nbsp;&nbsp;已参与 <font style="color:#F00; font-size:12px;"><?php echo $rsProducts["canyurenshu"];?></font> <br />
			注：<font style="color:#F00; font-size:12px;">因为商品添加后云购码已经生成成功,所以不能修改价格了</font>
			</div>
			<form class="r_con_form" id="product_edit_form" method="post" action="products_edit.php">
				...
				<div class="rows">
					...
					<select name='Category'>
						...
						<?php
						$DB->get("cloud_category","*","where Users_ID='".$_SESSION["Users_ID"]."' and Category_ParentID=0 order by Category_Index asc");
						...
						foreach($ParentCategory as $key=>$value){
							$DB->get("cloud_category","*","where Users_ID='".$_SESSION["Users_ID"]."' and Category_ParentID=".$value["Category_ID"]." order by Category_Index asc");
							if($DB->num_rows()>0){
          ...
          <span class="input" style="font-size:12px;">
              <input type="radio" value="0" name="Products_Status"<?php echo $rsProducts["Products_Status"]==0 ? ' checked' : '';?> /><label for="status_0"> 待审核 </label>&nbsp;&nbsp;
              <input type="radio" value="1" name="Products_Status"<?php echo $rsProducts["Products_Status"]==1 ? ' checked' : '';?> /><label for="status_1"> 通过审核 </label>
          </span>
          <div class="clear"></div>
        </div>
				<div class="rows">

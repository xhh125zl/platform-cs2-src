...
if(isset($_GET["action"]))
{
	if($_GET["action"]=="del")
	{
		$Flag=$DB->Del("shop_attribute","Users_ID='".$_SESSION["Users_ID"]."' and Attr_ID=".$_GET["Attr_ID"]);
		if($Flag)
		{
...
function get_type($typeid){
	global $DB;
	$r = $DB->GetRs("shop_product_type","*","where Type_ID='".$typeid."'");
	return $r ? $r['Type_Name'] : "全部";
}

//获取分类列表
$lists = array();
$rsTypes = $DB->get("shop_product_type","Type_ID,Type_Name","where Users_ID='".$_SESSION["Users_ID"]."' order by Type_Index asc");
...
<div id="iframe_page">
  <div class="iframe_content">
    ...
    <div class="r_nav">
      <ul>
        <li class="cur"><a href="shop_attr.php">产品属性</a></li>
        <li class=""><a href="shop_attr_add.php">添加属性</a></li>
      </ul>
    </div>
      ...
      <div class="property">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="mytable">
          ...
          <?php
			...
			$rsList = $DB->getPage("shop_attribute","*",$condition,10);
          ...
          <tr onMouseOver="this.bgColor='#D8EDF4';" onMouseOut="this.bgColor='';">
            ...
            <td align="center"><?php echo get_type($rsAttr["Type_ID"]); ?></td>

          	 ...
          	 <tbody>
          	 	<tr>
          	 		<td colspan="2">
          	 			...
          	 			<?php  if(!empty($order['Order_CartList'])){  ?>
          	 			<?php
                  foreach($order['Order_CartList'] as $kk => $vv){
                   foreach($vv as $k => $v){ 
                  ?>
          	 			<p><span  class="lbl">货品名：</span><span><?=$v['ProductsName']?>（￥ <?=$v['ProductsPriceX'] ?>）</span>
          	 			<span class="pd100">数量：<?=$v['Qty']?></span></p>
          	 			<p><span  class="lbl">货品费用：</span><span>￥  + 配送费用：￥ <?=isset($order['Order_Shipping']['Price'])?$order['Order_Shipping']['Price']:1?>  = 总金额：￥ <?=$order['Order_TotalAmount']?></span></p>

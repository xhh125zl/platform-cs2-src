    ...
    foreach ($ParentCategory as $key => $value) {
        ...
        if (!empty($catelist)) {
            ...
            foreach($catelist as $rsCategory){
                $strSelect  .= '<option value="' . $rsCategory["Category_ID"] . '"'.($Article["Category_ID"]==$rsCategory['Category_ID']?"selected":"").'>' . $rsCategory["Category_Name"] . '</option>';
            }
            $strSelect  .= '</optgroup>';
        } else {
            $strSelect  .= '<option value="' . $value["Category_ID"] . '"'.($Article["Category_ID"]==$value['Category_ID']?"selected":"").'>' . $value["Category_Name"] . '</option>';
        }
    }
}
?>

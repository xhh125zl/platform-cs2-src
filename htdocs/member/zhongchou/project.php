...
if(isset($_GET["action"])){
	if($_GET["action"]=="del"){
		$Flag=$DB->Del("zhongchou_project","usersid='".$_SESSION["Users_ID"]."' and itemid=".$_GET["itemid"]);
		if($Flag){
    ...
    <div id="project" class="r_con_wrap">
      ...
      <table border="0" cellpadding="5" cellspacing="0" class="r_con_table">
        ...
        <tbody>
          <?php
		  	$lists = array();
          	$DB->getPage("zhongchou_project","*","where usersid='".$_SESSION["Users_ID"]."' order by addtime desc",10);
		  	while($r=$DB->fetch_assoc()){
            	$lists[] = $r;
			}
			foreach($lists as $k=>$v){
				$item = $DB->GetRs("user_order","count(*) as num,sum(Order_TotalPrice) as amount","where Order_Type='zhongchou_".$v["itemid"]."' and Users_ID='".$_SESSION["Users_ID"]."'");
				$v["people"] = empty($item["num"]) ? 0 : $item["num"];
				$v["complete"] = empty($item["amount"]) ? 0 : $item["amount"];
				$item = $DB->GetRs("zhongchou_prize","count(*) as num","where usersid='".$_SESSION["Users_ID"]."' and projectid=".$v["itemid"]);
				$v["prize"] = $item["num"];
		  ?>
          <tr>
            ...
            <td nowrap="nowrap"><?php echo $v["status"]==0 ? '<font style="color:blue">进行中</font>' : '<font style="color:red">已过期</font>';?></td>

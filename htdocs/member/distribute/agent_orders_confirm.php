...
$rsOrder=$DB->GetRs("agent_order","*","where Users_ID='".$_SESSION['Users_ID']."' and Order_ID=".$OrderID);
...
if($_POST){
	...
	$Flag=$DB->Set("agent_order",$Data,"where Order_ID=".$OrderID);
	if($Flag){
		if($_POST["refuse"] == 1){
		echo '<script language="javascript">alert("申请已通过");window.location="agent_orders.php";</script>';
	}else{
		echo '<script language="javascript">alert("申请已回绝");window.location="agent_orders.php";</script>';
	}	
	}else{

    ...
    <div class="r_con_wrap">
      <form id="distribute_config_form" class="r_con_form" method="post" action="?">
        ...
        <div class="rows" id="Agent_Ratf_Row"<?php echo $rsConfig["Sha_Agent_Type"]==1 ? ' style="display:block"' : ' style="display:none"'?>>
        	<label>条件设置</label>
       		<span class="input" id="Agent_Ratf_Input">
				<?php if(!empty($distribute_level)):?>
                 等级<select name="Sha_Rate[sha][Level]">
				 <?php 
				    $Sha_Rate_list = json_decode($rsConfig['Sha_Rate'],TRUE);
				  ?>
				 <option value="0" <?php echo (isset($Sha_Rate_list['sha']['Level'])?$Sha_Rate_list['sha']['Level']:'')== 0 ? ' selected' : '';?>>---选择等级---</option>
				 <?php foreach($distribute_level as $key=>$level):?>				 
                   <option value="<?=$level['Level_ID']?>" <?php echo $level['Level_ID']==(isset($Sha_Rate_list['sha']['Level'])?$Sha_Rate_list['sha']['Level']:'') ? ' selected' : '';?>><?=$level['Level_Name']?></option>
				   <?php endforeach;?>
              </select>
			  <?php endif;?>
			  <?php if(!empty($dis_title_level)):?>
			  &nbsp;&nbsp;&nbsp;爵位<select name="Sha_Rate[sha][Protitle]">
			  <option value="0" <?php echo (isset($Sha_Rate_list['sha']['Protitle'])?$Sha_Rate_list['sha']['Protitle']:'')== 0 ? ' selected' : '';?>>---选择爵位---</option>
				 <?php foreach($dis_title_level as $key=>$title):?>
                   <option value="<?=$key?>" <?php echo $key==(isset($Sha_Rate_list['sha']['Protitle'])?$Sha_Rate_list['sha']['Protitle']:'') ? ' selected' : '';?>><?=$title['Name']?></option>
				   <?php endforeach;?>
              </select>
			  <?php endif;?>
			  &nbsp;&nbsp;&nbsp;自费金额<strong class="red">（元）</strong><input type="text" name="Sha_Rate[sha][Selfpro]" value="<?=isset($Sha_Rate_list['sha']['Selfpro'])?$Sha_Rate_list['sha']['Selfpro']:''?>" class="form_input" size="3" maxlength="10" notnull />
				 &nbsp;&nbsp;&nbsp;团队销售额<strong class="red">（元）</strong><input type="text" name="Sha_Rate[sha][Teampro]" value="<?=isset($Sha_Rate_list['sha']['Teampro'])?$Sha_Rate_list['sha']['Teampro']:''?>" class="form_input" size="3" maxlength="10" notnull />	   			
        	  &nbsp;&nbsp;&nbsp;申请价格<strong class="red">（元）</strong><input type="text" name="Sha_Rate[sha][price]" value="<?=isset($Sha_Rate_list['sha']['price'])?$Sha_Rate_list['sha']['price']:''?>" class="form_input" size="3" maxlength="10" notnull />
        	</span>
        	<div class="clear"></div>
         </div>
         <!-- 股东条件设置end -->

...
$BenefitsID=empty($_REQUEST['BenefitsID'])?0:$_REQUEST['BenefitsID'];
$rsBenefits=$DB->GetRs("user_card_benefits","*","where Users_ID='".$_SESSION["Users_ID"]."' and Benefits_ID=".$BenefitsID);
if($_POST)
{
	...
	$Flag=$DB->Set("user_card_benefits",$Data,"where Users_ID='".$_SESSION["Users_ID"]."' and Benefits_ID=".$BenefitsID);
	if($Flag)
	{
		echo '<script language="javascript">alert("修改成功");window.location="card_benefits.php";</script>';
	}else
	{
...
<div id="iframe_page">
  <div class="iframe_content">
    ...
    <div id="card_benefits" class="r_con_wrap"> 
      ...
      <form id="card_benefits_form" class="r_con_form" method="post" action="card_benefits_edit.php">
        <div class="rows">
          <label>内容标题</label>
          <span class="input">
          <input name="Title" value="<?php echo $rsBenefits['Benefits_Title'] ?>" type="text" class="form_input" size="40" maxlength="100" notnull>
        ...
        <div class="rows">
          ...
          <?php foreach($UserLevel as $k=>$v){
			  echo '<option value="'.$k.'"'.($rsBenefits['Benefits_UserLevel']==$k?' selected':'').'>'.$v['Name'].'</option>';
		  }?>
          </select>
          </span>
          <div class="clear"></div>
        ...
        <div class="rows">
          <label>显示时间</label>
          <span class="input time">
          <input name="Time" type="text" value="<?php echo date("Y/m/d H:i:s",$rsBenefits["Benefits_StartTime"])." - ".date("Y/m/d H:i:s",$rsBenefits["Benefits_EndTime"]) ?>" class="form_input" size="40" readonly="readonly" notnull />
          <font class="fc_red">*</font> </span>
          <div class="clear"></div>
        </div>
        <div class="rows">
          <label>详细内容</label>
          <span class="input">
          <textarea class="ckeditor" name="Description" style="width:500px; height:300px;"><?php echo $rsBenefits['Benefits_Description'] ?></textarea>
          </span>
          <div class="clear"></div>
        </div>
        <div class="rows">
        ...
        <input type="hidden" name="BenefitsID" value="<?php echo $rsBenefits['Benefits_ID'] ?>">
      </form>
    </div>
  </div>
</div>

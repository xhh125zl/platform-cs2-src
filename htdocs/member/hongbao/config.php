...
if(!$rsConfig){
	...
	$DB->Add("hongbao_config",$Data);
	$rsConfig = $Data;
}
$json=$DB->GetRs("wechat_material","*","where Users_ID='".$_SESSION["Users_ID"]."' and Material_Table='hongbao' and Material_TableID=0 and Material_Display=0");
if(empty($json)){
	$Material=array(
		"Title"=>"抢红包",
		"ImgPath"=>"/static/api/images/cover_img/hongbao.jpg",
		"TextContents"=>"",
		"Url"=>"/api/".$_SESSION["Users_ID"]."/hongbao/"
	);
	$Data=array(
		"Users_ID"=>$_SESSION["Users_ID"],
		"Material_Table"=>"hongbao",
		"Material_TableID"=>0,
		"Material_Display"=>0,
		"Material_Type"=>0,
...
$rsKeyword=$DB->GetRs("wechat_keyword_reply","*","where Users_ID='".$_SESSION["Users_ID"]."' and Reply_Table='hongbao' and Reply_TableID=0 and Reply_Display=0");
if(empty($rsKeyword)){
...
if($_POST)
{
	...
	$Data=array(
		...
		"supply"=>$_POST["supply"],
		"fromtime"=>$fromtime,
		"pertime"=>empty($_POST["pertime"]) ? 60 : $_POST["pertime"],
		"totime"=>$totime
	);
	$Set=$DB->Set("hongbao_config",$Data,"where usersid='".$_SESSION["Users_ID"]."'");
	$flag=$flag&&$Set;
	$Data=array(
		"Reply_Keywords"=>$_POST["Keywords"],
		"Reply_PatternMethod"=>isset($_POST["PatternMethod"])?$_POST["PatternMethod"]:0
	);
	$Set=$DB->Set("wechat_keyword_reply",$Data,"where Users_ID='".$_SESSION["Users_ID"]."' and Reply_Table='hongbao' and Reply_TableID=0 and Reply_Display=0");
	$flag=$flag&&$Set;
	$Material=array(
		"Title"=>$_POST["Title"],
	...
	$Set=$DB->Set("wechat_material",$Data,"where Users_ID='".$_SESSION["Users_ID"]."' and Material_Table='hongbao' and Material_TableID=0 and Material_Display=0");
	$flag=$flag&&$Set;
	if($flag)
	{
		mysql_query("commit");
...
<div id="iframe_page">
  <div class="iframe_content">
	...
	<script language="javascript">$(document).ready(hongbao_obj.config_form_init);</script>
	    ...
	    <table border="0" cellpadding="0" cellspacing="0">
            <tr>
                <td width="50%" valign="top">
                    ...
                    <input type="text" class="input" name="Name" value="<?php echo $rsConfig ? $rsConfig['name'] : ''?>" maxlength="30" notnull />
                </td>
                <td width="50%" valign="top">
                    <h1><span class="fc_red">*</span> <strong>活动时间</strong></h1>
                    <input name="Time" type="text" value="<?php echo $rsConfig['fromtime']>0 ? date("Y/m/d H:i:s",$rsConfig['fromtime']) : date("Y/m/d H:i:s") ?> - <?php echo $rsConfig['totime']>0 ? date("Y/m/d H:i:s",$rsConfig['totime']) : date("Y/m/d H:i:s",strtotime("+7 day")) ?>" class="form_input" size="40" readonly notnull />
                </td>
            </tr>
            <tr>
                <td width="50%" valign="top">
                    <h1><span class="fc_red">*</span> <strong>红包提供方</strong><font style="color:#ff0000; font-size:12px;">(例如：网中网)</font></h1>
                    <input type="text" class="input" name="supply" value="<?php echo $rsConfig ? $rsConfig['supply'] : ''?>" maxlength="30" notnull />
                </td>
                <td width="50%" valign="top">&nbsp;
                    <h1><span class="fc_red">*</span> <strong>时间间隔</strong><font style="color:#ff0000; font-size:12px;">(一小时60分钟，一天1440分钟)</font></h1>
                    每人每 <input type="text" class="input" name="pertime" value="<?php echo $rsConfig ? $rsConfig['pertime'] : ''?>" style="width:60px; text-align:center" notnull /> 分钟
                </td>
            </tr>
        </table>
            ...
            <td><h1><strong>触发信息设置</strong></h1>
              <div class="reply_msg">
                ...
                <div class="m_right"> <span class="fc_red">*</span> 图文消息封面<span class="tips">（大图尺寸建议：640*360px）</span><br />
                  ...
                  <br />
                  <div class="img" id="ImgDetail"><img src="<?php echo empty($rsMaterial["ImgPath"])?"/static/api/images/cover_img/hongbao.jpg":$rsMaterial["ImgPath"]; ?>" width="640" height="360"></div>
                </div>
                <div class="clear"></div>
              </div>

...
if($_POST){
	if(empty($_POST["id"])){
		...
		if(!empty($_POST["emppwd"])){
				$Data['employee_pass'] = md5($_POST['emppwd']);
			}
		
		if($DB->set('users_employee',$Data,' where id='.$_POST['id'].' and users_account="'.$_SESSION['Users_Account'].'"')){
				echo '<script language="javascript">alert("修改成功");window.location="emplist.php";</script>';
			}else{
				echo mysql_error();
				echo '<script language="javascript">alert("修改失败");window.location="emplist.php";</script>';
			}
  ...
  <div class="iframe_content">   
    <div class="r_nav">
      <ul>
		 ...
		 <li class="red"><strong>员工修改</strong></li>
      </ul>
    </div>
    <div id="products" class="r_con_wrap">
      ...
      <form class="r_con_form" method="post" action="?" id="emp_add_form">
       <div class="rows">
                <label>员工称呼</label>
                <span class="input"><input type="text" name="empname" class="form_input" required id="empname" value='<?php echo !empty($employee['employee_name'])?$employee['employee_name']:''?>' /> <font class="fc_red">*</font></span>
                <div class="clear"></div>
            </div>
        <div class="rows">
                <label>登录帐号</label>
                <span class="input"><input type="text" name="empaccount" class="form_input" disabled="true" id="empaccount" value='<?php echo !empty($employee['employee_login_name'])?$employee['employee_login_name']:''?>' /> <font class="fc_red">*</font></span>
			...
			<div class="rows">
                <label>是否启用</label>
                <span class="input">
                    <label><input name="status" type="radio" value="1" <?php if(!empty($employee['status']) && $employee['status'] == 1) echo "checked";?>>启用</label>

...
$rsConfig=$DB->GetRs("zhuli","Awordrules","where Users_ID='".$_SESSION["Users_ID"]."'");
if(empty($rsConfig)){
...
if($_POST){	
	$Data=array(		
		"Awordrules"=>$_POST['Description']
	);
	$Flag=$DB->Set("zhuli",$Data,"where Users_ID='".$_SESSION["Users_ID"]."'");
	if($Flag){
		echo '<script language="javascript">alert("设置成功");window.location="awordrules.php";</script>';
	}else{
		echo '<script language="javascript">alert("设置失败");window.location="awordrules.php";</script>';
	}
	exit;
}
?>
  ...
  <div class="iframe_content">
    ...
    <div class="r_nav">
      <ul>
		...
		<li class="cur"><a href="awordrules.php">兑奖规则</a></li>
    ...
    <div id="rules" class="r_con_wrap">
      ...
      <form class="r_con_form" method="post" action="awordrules.php" id="form_submit">
        <div class="rows" id="Description_rows">
          <label>兑奖规则</label>
          <span class="input">
          <textarea name="Description" style="width:100%;height:400px;visibility:hidden;"><?php echo $rsConfig["Awordrules"] ?></textarea>
          </span>
          <div class="clear"></div>
        </div>
        <div class="rows">
